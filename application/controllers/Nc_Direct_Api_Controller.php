...
class Nc_Direct_Api_Controller extends Awx_Controller
{
    /**
     * Constructor
     *
    ...
    public function direct_api()
    {
        ...
        $this->load->view( 'nc_direct_api_checkout', $this->vars );
    }
    ...
    public function do_checkout_direct_api()
    {
        ...
        $order = [
            ...
            'return_url' => site_url( 'nc-direct-api-callback' )
        ];
        ...
        $payment_detail = [
            ...
            'payment_method_options' => [
                'card' => [
                    'auto_capture'  => true,
                    'three_ds'      => [
                        'return_url' => site_url( 'nc-direct-api-callback/' . $intent[ 'id' ] ) . '?c=' . $client_id . '&k=' . $api_key
                    ]
                ]
            ]
        ];
    ...
    public function three_ds_device()
    {
        ...
        $this->vars[ 'data' ]   = $this->input->get();

        $this->load->view( 'nc_device_3ds', $this->vars );
    }
    ...
    public function three_ds_callback( $intent_id = '' )
    {
        ...
        {
            redirect( site_url( 'nc-direct-api-3ds-result/1' . '?id=' . $intent_id . '&c=' . $client_id . '&k=' . $api_key ) );
        ...
        if ( FALSE !== $res )
        {
            redirect( site_url( 'nc-direct-api-3ds-result/0' . '?id=' . $intent_id . '&c=' . $client_id . '&k=' . $api_key . ( isset( $res[ 'provider_original_response_code' ] ) ? '&code=' . $res[ 'provider_original_response_code' ] : '' ) ) );
    ...
    public function three_ds_result( $res = 1 )
    {
        ...
        if ( ! empty( $id ) )
        {
            $result_uri .= '?id=' . $id . '&c=' . $c . '&k=' . $k . '&m=nc-direct-api';
        }
